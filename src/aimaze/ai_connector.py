...
def generate_random_event(location_context: str) -> GameEvent | None:
    """
    Genera un evento aleatorio ligero favoreciendo puzzles.
    Implementación local simple (sin LLM) para el MVP del Paso 1.6.
    """
    # 30% de probabilidad de no generar evento
    if random.random() < 0.30:
        return None

    # Favorecer puzzles (60%)
    r = random.random()
    if r < 0.6:
    ...
    # 40% restantes: obstáculos sencillos
...
def generate_advanced_main_path(start: tuple, end: tuple, width: int, height: int, target_length: int) -> list:
    ...
    if not paths_found:
        ...
        print("Warning: No se encontraron caminos, usando camino directo")
    ...
    if target_paths:
        ...
        print(
            f"Camino seleccionado: longitud {len(selected_path)}/{target_length} (objetivo)")
    else:
        ...
        print(
            f"Fallback: camino más largo encontrado: longitud {len(selected_path)}/{target_length} (objetivo)")
...
def calculate_smart_path_length(start: tuple, end: tuple, width: int, height: int) -> int:
    ...
    # Usar distribución beta para favorecer valores intermedios
...
def generate_dungeon_layout() -> Dungeon:
    ...
    target_path_length = calculate_smart_path_length(
        start_coords, exit_coords, width, height)

    print(
        f"Generando mazmorra determinista: {width}x{height} ({total_cells} celdas totales)")
    print(f"Entrada: {start_coords}, Salida: {exit_coords}")
    print(f"Distancia mínima: {min_distance}, Máximo posible: {total_cells}")
    print(
        f"Longitud objetivo del camino: {target_path_length} habitaciones ({target_path_length / total_cells * 100:.1f}%)")

    # Generar camino principal con longitud objetivo
    path_rooms = generate_advanced_main_path(
        start_coords, exit_coords, width, height, target_path_length)
    ...
    dungeon = create_dungeon_from_rooms(
        all_rooms, width, height, start_coords, exit_coords)
